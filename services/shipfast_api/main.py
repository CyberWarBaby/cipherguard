...
def verify_api_key(api_key: Optional[str] = Security(api_key_header)):
    """
    Validates incoming request's X-API-Key header against SHIPFAST_API_KEY environment variable.
    Returns HTTP 401 Unauthorized if the header is missing or invalid.
    """
    expected_key = os.getenv("SHIPFAST_API_KEY")
    ...
    if not expected_key or not hmac.compare_digest(api_key.encode("utf-8"), expected_key.encode("utf-8")):
        raise HTTPException(
            status_code=status.HTTP_401_UNAUTHORIZED,
...
# ------------------------------------------------------------------------------
@app.get("/orders/{order_id}", response_model=OrderResponse, dependencies=[Depends(verify_api_key)], tags=["Orders"])
...
@app.post("/orders", response_model=OrderResponse, status_code=status.HTTP_201_CREATED, dependencies=[Depends(verify_api_key)], tags=["Orders"])
...
@app.get("/customers/{customer_id}/address", response_model=CustomerAddressResponse, dependencies=[Depends(verify_api_key)], tags=["Customers"])
